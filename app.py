...
def upload():
    ...
    if file_person and allowed_file(file_person.filename) and file_cloth and allowed_file(file_cloth.filename):
        ...
        # ..... Image parser ..... # 
        cmd_parse = "python tryon_utils/inference.py --loadmodel checkpoints/inference.pth --img_path " + filename_person + " --output_path img/ --output_name "+ filename_person
        ...
        cmd_gmm = "python tryon_utils/test.py --name GMM --stage GMM --workers 4 --datamode test --data_list test_samples_pair.txt --checkpoint checkpoints/GMM/gmm_final.pth"
        ...
        cmd_tom = "python tryon_utils/test.py --name TOM --stage TOM --workers 4 --datamode test --data_list test_samples_pair.txt --checkpoint checkpoints/TOM/tom_final.pth"
        subprocess.call(cmd_tom, shell=True) 
        
        # encoded_string = ""
        # with open("result/TOM/test/try-on/"+filename_person, "rb") as image_file:
        #     encoded_string = base64.b64encode(image_file.read())
        # response = make_response(encoded_string, 200)
        # response.mimetype = "text/plain"
        # return response

        return send_file("result/TOM/test/try-on/"+filename_person, mimetype='image/jpg')
